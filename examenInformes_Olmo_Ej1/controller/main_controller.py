    ...
    def cargar_tabla(self):
        try:
            ...
            for indice, datos in enumerate(rows):
                ...
                self.tableWidget.setItem(indice, 2, QTableWidgetItem(str(datos["total_semana"])))
                self.tableWidget.setItem(indice, 3, QTableWidgetItem(str(datos["max_dia"])))
                self.tableWidget.setItem(indice, 4, QTableWidgetItem(str(datos["exceso"])))
        except Exception as e:
            QMessageBox.critical(self, "ERROR", "No se ha podido cargar la tabla.")

    def generar_informe_control(self):
        try:
            file_path, _ = QFileDialog().getSaveFileName(self, "Guardando Informe de Control de Horas", "ejercicio1_informe.pdf", "PDF Files (*.pdf)")
            if file_path:
